...
export default function Dashboard() {
  ...
  const statCards = [
    { label: "Subjects", value: stats?.subjectCount || 0, icon: BookOpen, color: "text-blue-500", bg: "bg-blue-500/10" },
    { label: "Topics", value: stats?.topicCount || 0, icon: Layers, color: "text-indigo-500", bg: "bg-indigo-500/10" },
    { label: "Notes", value: stats?.noteCount || 0, icon: FileText, color: "text-violet-500", bg: "bg-violet-500/10" },
    { label: "Videos", value: stats?.videoCount || 0, icon: Video, color: "text-fuchsia-500", bg: "bg-fuchsia-500/10" },
    { label: "Pending Todos", value: stats?.pendingTodoCount || 0, icon: CheckSquare, color: "text-rose-500", bg: "bg-rose-500/10" },
    { label: "Key Formulas", value: stats?.keyFormulaCount || 0, icon: FunctionSquare, color: "text-amber-500", bg: "bg-amber-500/10" },
  ];
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
              ...
              {dashboardTodos && dashboardTodos.length > 0 ? (
                <div className="space-y-1">
                  {dashboardTodos.map((todo) => (
                    <div key={todo.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-accent/50 transition-colors">
                      <Checkbox 
                        checked={todo.completed} 
                        onCheckedChange={(c) => handleToggleTodo(todo.id, !!c)} 
                        className="mt-0.5"
                      />
              ...
              {importantFormulas && importantFormulas.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {importantFormulas.map((formula) => (
                    <div key={formula.id} className="p-4 rounded-xl border border-amber-200 dark:border-amber-900 bg-amber-50/50 dark:bg-amber-900/10 relative overflow-hidden group">
        ...
        {/* Right Column: Events & Quick Notes */}
                  ...
                  {upcomingEvents.map((event) => (
                    <div key={event.id} className="flex gap-4">
                      ...
                      <div className="flex-1 pb-4 border-b border-border last:border-0 last:pb-0">
                        ...
                        {event.subjectName && (
                          <p className="text-xs text-muted-foreground mt-1">{event.subjectName}</p>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              {recentNotes && recentNotes.length > 0 ? (
                <div className="space-y-3">
                  {recentNotes.map((note) => (
                    <div 
                      key={note.id} 
                      className="p-3 rounded-r-lg rounded-l-sm bg-card border border-border border-l-4 shadow-sm"
